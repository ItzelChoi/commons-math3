...
public class MaxTest extends StorelessUnivariateStatisticAbstractTest {
    ...
    public void testNormalValues() {
        double[] testArray = {2.3d, 5.264d, 0.254d,
                15};
        Max max = new Max();
        Assert.assertTrue(Double.isNaN(max.getResult()));
        max.increment(testArray[0]);
        Assert.assertEquals(2.3d, max.getResult(), 0);
        max.increment(testArray[1]);
        Assert.assertEquals(5.264d, max.getResult(), 0);
        max.increment(testArray[2]);
        Assert.assertEquals(5.264d, max.getResult(), 0);
        max.increment(testArray[3]);
        Assert.assertEquals(15, max.getResult(), 0);
        Assert.assertEquals(15, max.evaluate(testArray), 0);
    }
}
